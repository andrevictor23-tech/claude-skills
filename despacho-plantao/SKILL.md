...
description: 'Emite o despacho do Delegado de plantão a partir do fato narrado, no padrão real da Delegacia de Alta Floresta/MT, decidindo entre APF, TCO, BOC, instaurar inquérito ou deixar de instaurar. Use ao apresentar ocorrência, prisão, abordagem ou flagrante pedindo o despacho, o encaminhamento ou as providências, ou ao perguntar qual medida cabe ("ratifico a voz de prisão?", "esse caso é Maria da Penha?", "deixo de instaurar?"). Cobre violência doméstica, protetiva, lesão, ameaça, estupro de vulnerável, drogas, arma, roubo, furto, extorsão e crimes em geral. Não use para relatório final de IP (relatorio-final-ip) nem para a representação em si (representacao-cautelar).'
---
