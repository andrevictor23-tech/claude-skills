...
description: Redige representações da Autoridade Policial dirigidas ao Juízo, no padrão real da Delegacia de Polícia de Alta Floresta/MT e do NEAMV. Cobre prisão preventiva, prisão temporária, busca e apreensão domiciliar, quebra/afastamento de sigilo de dados telefônicos e telemáticos, interceptação telefônica, sigilo bancário e fiscal, e acesso a dados de dispositivos apreendidos. Use SEMPRE que o usuário pedir para representar, redigir representação, pedir preventiva, pedir temporária, pedir busca e apreensão, pedir quebra de sigilo, interceptação, afastamento de sigilo, ou disser "representa pela preventiva", "monta a representação", "faz o pedido de busca", "quebra o sigilo do alvo", "pede a interceptação", "representa pelo acesso ao celular apreendido". Também use quando o despacho de plantão já sinalizou "é caso para REPRESENTAÇÃO PELA PRISÃO PREVENTIVA" e o usuário quiser a peça completa, quando o usuário entregar autos ou fatos pedindo para avaliar quais medidas cautelares cabem ("olha esses autos e vê o que cabe", "o que dá pra pedir nesse caso"), e quando pedir para adicionar ou ingerir modelos de representação na base de conhecimento da skill. Não use para despacho de plantão (use despacho-plantao) nem para relatório final de IP (use relatorio-final-ip).
---
...
**Banco de modelos do usuário**: além das referências, o usuário mantém modelos reais das suas peças em `assets/modelos/`, indexados em `references/catalogo-modelos.md`. Consulte o catálogo (nunca a pasta inteira) e, havendo modelo compatível com a medida e a hipótese, carregue-o e use-o como esqueleto preferencial: a fraseologia real do usuário prevalece sobre os blocos genéricos das referências. Para adicionar modelos novos, siga o procedimento de ingestão descrito no próprio catálogo.
