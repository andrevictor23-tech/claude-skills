...
description: Redige representações da Autoridade Policial ao Juízo, no padrão real da Delegacia de Alta Floresta/MT e do NEAMV, com base em banco de modelos reais do próprio Delegado. Cobre prisão preventiva, temporária, busca e apreensão, afastamento de sigilo (telefônico, telemático, bancário, fiscal), interceptação, acesso a dados de dispositivos apreendidos e medidas assecuratórias, com os ofícios judiciais a provedores de internet, redes sociais, aplicativos de mensagens e operadoras. Use quando o usuário pedir para representar ("representa pela preventiva", "monta a representação", "faz o pedido de busca", "quebra o sigilo do alvo", "pede a interceptação", "representa pelo acesso ao celular apreendido"), quando entregar autos pedindo quais medidas cabem ("vê o que cabe nesses autos"), quando o despacho de plantão sinalizou representação, ou para adicionar novos modelos à base. Não use para despacho de plantão (use despacho-plantao) nem para relatório final de IP (use relatorio-final-ip).
---
