...
description: Redige representações da Autoridade Policial dirigidas ao Juízo, no padrão real da Delegacia de Polícia de Alta Floresta/MT e do NEAMV, com base em banco de modelos reais do próprio Delegado. Cobre prisão preventiva, temporária, busca e apreensão, afastamento de sigilo (telefônico, telemático, bancário, fiscal), interceptação, acesso a dados de dispositivos apreendidos e medidas assecuratórias, incluindo os ofícios judiciais a provedores de aplicação de internet, redes sociais, aplicativos de mensagens, transporte ou entrega e operadoras de telefonia. Use quando o usuário pedir para representar ou redigir representação ("representa pela preventiva", "monta a representação", "faz o pedido de busca", "quebra o sigilo do alvo", "pede a interceptação", "representa pelo acesso ao celular apreendido"), quando entregar autos pedindo para avaliar quais medidas cabem ("vê o que cabe nesses autos"), quando o despacho de plantão já sinalizou representação, ou para adicionar novos modelos à base da skill. Não use para despacho de plantão (use despacho-plantao) nem para relatório final de IP (use relatorio-final-ip).
---
